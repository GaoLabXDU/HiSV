...
if __name__ == '__main__':
    ...
    parser = OptionParser()
    parser.add_option('-o', '--output', default='/home/li/Test_data/result', help='path of out file')
    parser.add_option('-l', '--length', default='/home/li/Test_data/ref.len', help='length file of reference')
    parser.add_option('-f', '--hicfile', default='/home/li/Test_data/hic_data', help='path of hic matrix file (The matrix are named chr*.txt and chr*_chr*.txt) ')
    parser.add_option('-a', '--intra_binsize', type='int', default=50000, help='the binsize of intra chromosome')
    parser.add_option('-e', '--inter_binsize', type='int', default=50000, help='the binsize of inter chromosome')
    parser.add_option('-w', '--window', type='int', default=10)
    parser.add_option('-c', '--cutoff', type='float', default=0.6)
    ...
    intra_binSize = opts.intra_binsize
    inter_binSize = opts.inter_binsize
    win = opts.window
    cutoff = opts.cutoff
    ...
    chr_id = read_reflen(chr_len_file)
    ...
    for i in range(chr_count):
        for j in range(i, chr_count):
            ...
            score_result_file = os.path.join(output, '%s_%s_HiSV_score.txt' % (chr_id[i], chr_id[j]))
            if i == j:
                # intra_chromosomal
            ...
            # with open(outfile, 'a') as out:
                ...
                # intrer_chromosomal
                matrix_file = os.path.join(hicfile, '%s_%s.txt' % (chr_id[i], chr_id[j]))
                print(matrix_file)
                contact_matrix = np.loadtxt(matrix_file)
