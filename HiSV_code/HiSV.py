...
if __name__ == '__main__':
    ...
    parser.add_option('-f', '--hicfile', default='./test_matrix', help='path of hic matrix file (The matrix are named chr*.txt and chr*_chr*.txt) ')
    ...
    for i in range(chr_count):
        for j in range(i, chr_count):
            ...
            score_result_file = os.path.join(output, 'sample_%skb_%s_%s_HiSV_score.txt' % (str(binSize//1000), chr_id[i], chr_id[j]))
            if i == j:
                # intra_chromosomal
                matrix_file = os.path.join(hicfile, 'sample_%skb_%s_%s_matrix.txt' % (str(binSize//1000), chr_id[i], chr_id[j]))
