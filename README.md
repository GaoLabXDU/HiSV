...
Note: the input hic file format should be mcool, cool, hic or bam.
```
...
```
The storage path of the matrix file is as follows:
```
...
```
## Calling SVs by HiSV
When the matrix files are prepared, the user can use the ``HiSV`` script to identify SVs from Hi-C matrix files. Here, we provide an example Hi-C dataset to guide users through the whole process. The dataset contained two chromsomes, 9 and 13, and these two chromosomes in the K562 sample covered some of SVs.  
