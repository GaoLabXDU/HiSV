...
if __name__ == '__main__':
    ...
    parser = OptionParser()
    parser.add_option('-c', '--case', default='/media/li/Data/GM12878/K562_remove_gap/', help='path of cancer matrix file')
    parser.add_option('-l', '--length', default='/home/li/PycharmProjects/HiSV/hg19.len', help='length file of reference')
    parser.add_option('-b', '--binsize', type='int', default=5000)
    parser.add_option('-n', '--normal',  default='/media/li/Data/GM12878/GM12878_remove_gap/', help="path of normal matrix file")
    ...
    for k in range(3, 4):
        ...
        case_file = casefile + str(k+1) + ".txt"
        case_matrix = np.loadtxt(case_file)

        control_file = normalfile + str(k + 1) + ".txt"
        control_matrix = np.loadtxt(case_file)
